...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for team in teams:
            self.stdout.write(f"- {team.name} (Members: {team.members.count()})")

        # Verify activities
        activities = Activity.objects.all()
        self.stdout.write(f"Activities: {activities.count()}")
        for activity in activities:
            self.stdout.write(f"- {activity.activity_type} by {activity.user.username} ({activity.duration})")
        ...
        self.stdout.write(self.style.SUCCESS('Database verification complete.'))